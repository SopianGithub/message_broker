...
class RetryAPI extends Job
{
    ...
    public function handle()
    {
        ...
        if($response->status == 200){
            ...
            $item = new InvoiceModel(['payload_id' => $this->log['id'], 'response' => json_encode($response->content), 'is_done' => true, 'is_retry' => false]);

            event(new PusherLogsEvent($item));
        }else{
            Log::error('Errors : '. $response->error );

            $item = new InvoiceModel(['payload_id' => $this->log['id'], 'response' => $response->error, 'is_done' => false, 'is_retry' => false, 'is_feedback' => true]);
